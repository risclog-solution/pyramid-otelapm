...
def make_app(endpoint, **extra_settings):
    ...
    config.add_view(lambda x: {"status": "ok"}, route_name="index", renderer="json")
    config.add_route("ignored", "/mark_log")
    config.add_view(lambda x: {"status": "ok"}, route_name="ignored", renderer="json")
...
def test_get_transaction_name():
    ...
    assert "GET /foo/{id}" == pyramid_otelapm.get_transaction_name(FakeRequest())
...
def test_get_transaction_name_excludes_fanstatic_resources():
    ...
    assert "GET resources/*subpath" == pyramid_otelapm.get_transaction_name(
        FakeRequest()
    )
